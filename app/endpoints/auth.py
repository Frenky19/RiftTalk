...
async def discord_oauth_login_url(
    ...
    if settings.is_client:
        try:
            return JSONResponse(await remote_api.discord_login_url(summoner_id))
        except RemoteAPIError as e:
            raise HTTPException(status_code=502, detail=str(e))

    # --- Server/local OAuth (kept for compatibility; normally server mode uses /api/public/discord/*) ---
...
async def discord_oauth_callback(
    ...
    if settings.is_client:
        return HTMLResponse('<h3>This client is in remote mode. OAuth callback is handled by the server.</h3>', status_code=400)
