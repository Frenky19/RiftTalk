...
class LCUConnector:
    ...
    def _get_lockfile_path(self) -> str:
        ...
        return os.path.join(base_path, "Riot Games", "Riot Client", "Config", "lockfile")
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(self.lockfile_path):
                raise LCUException("Lockfile not found. Is the client running?")
            ...
            async with self.session.get(test_url) as response:
                if response.status == 200:
                    ...
                    raise LCUException(f"LCU connection test failed: {response.status}")
    ...
    async def make_request(self, method: str, endpoint: str, data: Optional[Any] = None) -> Any:
        ...
        try:
            async with self.session.request(method, url, json=data) as response:
                if response.status == 200:
                    ...
                    raise LCUException(f"LCU API error: {response.status} - {await response.text()}")
        except Exception as e:
